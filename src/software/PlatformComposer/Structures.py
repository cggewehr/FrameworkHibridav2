...
class Structure:
    ...
    def __init__(self, StructureType, AmountOfPEs):
        ...
        self.AddressInBaseNoC = None  # To be set when Platform.addStructure() is called
        ...
        if str(StructureType).casefold() == "crossbar":
            ...
            self.PEs = [PE(PEPos = i, AddressInBaseNoC = None, PosInStruct = i, CommStructure = "Crossbar", InjectorClockFrequency = None) for i in range(self.AmountOfPEs)]
        elif str(StructureType).casefold() == "bus":
            self.PEs = [PE(PEPos = i, AddressInBaseNoC = None, PosInStruct = i, CommStructure = "Bus", InjectorClockFrequency = None) for i in range(self.AmountOfPEs)]
        else:
