...
class Platform:
    ...
    def setAllocationMap(self, AllocationMap):
        ...
        if len(AllocationMap) > self.AmountOfPEs:
            print("Warning: Amount of PEs in given AllocationMap <" + str(len(AllocationMap)) + "> exceeds amount of PEs in Platform (" + str(self.AmountOfPEs) + "). Truncating it to " + str(self.AmountOfPEs))
        ...
        for PEPos, ThreadInAllocMap in enumerate(AllocationMap[0:self.AmountOfPEs]):
            ...
            if isinstance(ThreadInAllocMap, Thread):
                ...
                try:
                    ...
                    print("Warning: Given Allocation Map defines an idle Thread at PEPos <" + str(PEPos) + ">, which exceeds Platform AmountOfPEs, <" + str(self.AmountOfPEs) + ">")
    ...
    def setClusterClocks(self, ClusterClocks):
        ...
        if len(ClusterClocks) < self.BaseNoCDimensions[0]*self.BaseNoCDimensions[1]:
            print("Error: ClusterClocks size less than expected. <" + str(len(ClusterClocks)) + "> < <" + str(self.BaseNoCDimensions[0]*self.BaseNoCDimensions[1]) + ">")
            exit(1)
        if len(ClusterClocks) > self.BaseNoCDimensions[0]*self.BaseNoCDimensions[1]:
            print("Warning: ClusterClocks size greater than expected, truncating it to <" + str(self.BaseNoCDimensions[0]*self.BaseNoCDimensions[1]) + ">")
        
        if isinstance(ClusterClocks, dict) and "ClusterClockPeriods" in ClusterClocks.keys():
            self.ClusterClocks = ClusterClocks["ClusterClockPeriods"][0:self.BaseNoCDimensions[0]*self.BaseNoCDimensions[1]]

        elif isinstance(ClusterClocks, dict) and "ClusterClockFrequencies" in ClusterClocks.keys():
            self.ClusterClocks = [float(1000/ClusterClocks["ClusterClockPeriods"][Cluster]) for Cluster in ClusterClocks["ClusterClockPeriods"][0:self.BaseNoCDimensions[0]*self.BaseNoCDimensions[1]]]  # MHz -> ns
    ...
    # WIP: Defines cluster clocks from computed link usage
