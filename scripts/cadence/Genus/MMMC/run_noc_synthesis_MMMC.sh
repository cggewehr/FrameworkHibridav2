...
then 
    ...
    echo "Usage: sh run_synthesis.sh <PROJECT_DIR> <CLOCK_PERIOD (in nanoseconds, number only)> <X_DIM> <Y_DIM>"
    echo $#
	exit 1
fi
...
genus -log genus.log -f ${SynthProjectDir}/synthesis/scripts/GenusNoCStandaloneMMMC.tcl
