...
then 
    ...
    echo "Usage: sh run_synthesis.sh <PROJECT_DIR> <CLOCK_PERIOD (in nanoseconds, number only)> <AMOUNT_OF_PES> <BRIDGE_BUFFER_SIZE>"
	exit 1
fi
...
genus -log genus.log -f ${SynthProjectDir}/synthesis/scripts/GenusCrossbarStandaloneMMMC.tcl
